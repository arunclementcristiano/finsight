...
def handler(event, context):
    try:
        ...
        # ----------------------- NEW EXPENSE API ROUTES (for frontend compatibility) -----------------------
        
        # Create expense (POST /expenses) — body: { amount, category, description, date }
        if route_key == "POST /expenses":
            user_id = body.get("userId", "demo")  # Default to demo for now
            amount = body.get("amount")
        ...
        if route_key == "GET /expenses":
            user_id = qs.get("userId", "demo")  # Default to demo for now
            start = qs.get("start")
            end = qs.get("end")
