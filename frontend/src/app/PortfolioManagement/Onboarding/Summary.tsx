...
export default function Summary({ plan }: SummaryProps) {
	...
	const chartOptions = {
		plugins: {
			legend: { display: true, position: "bottom" as const, labels: { font: { size: 14 } } },
			tooltip: { callbacks: { label: function(context: any) { return `${context.label}: ${context.parsed}%`; } } },
		},
		cutout: "70%",
		responsive: true,
			...
			<div className="w-full max-w-2xl bg-white rounded-xl shadow-lg p-8 flex flex-col items-center">
				...
				<div className="w-full flex flex-col sm:flex-row gap-4 mt-8 justify-center">
					<button
						...
						onClick={() => { setPlan(plan as any); alert("Plan saved!"); }}
					>
						Save Plan
					</button>
					<button
