		...
		return (
			<Card className="w-full">
				<CardContent>
					...
					{helperText && (
						<div className={`flex items-start gap-1.5 ${compact ? "mb-2 text-[11px]" : "mb-4 text-xs"} text-muted-foreground`}>
							<Info className="mt-0.5 h-3.5 w-3.5 shrink-0"/><span>{helperText}</span>
						</div>
					)}
					<div className="mt-4">
						<input
	...
	return (
		<Card className="w-full">
			<CardContent>
				<div className={`${compact ? "mb-1 text-base" : "mb-2 text-lg"} font-semibold`}>{questionText}</div>
				{helperText && (
					<div className={`flex items-start gap-1.5 ${compact ? "mb-2 text-[11px]" : "mb-4 text-xs"} text-muted-foreground`}><Info className="mt-0.5 h-3.5 w-3.5 shrink-0"/><span>{helperText}</span></div>
				)}
