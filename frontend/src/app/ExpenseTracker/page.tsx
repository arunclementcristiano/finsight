            ...
            <div className="grid grid-cols-3 gap-4 text-sm">
              ...
              <div>
                <div className="text-muted-foreground flex items-center gap-2">
                  Category
                  {typeof msg.suggestion?.AIConfidence === "number" ? (
                    <span className="text-xs text-muted-foreground">AI {Math.round((msg.suggestion.AIConfidence ?? 0) * 100)}%</span>
                  ) : null}
                </div>
                    ...
                    {msg.suggestion.options.map((opt) => (
                      <button
                        ...
                        className={cn(
                          ...
                          category === opt ? "bg-[var(--color-ring)] text-white border-transparent" : "border-input"
                        )}
                      >
                        {opt}
                      </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        <div className="md:col-span-8 space-y-4">
          <Card>
            <CardContent>
              ...
              {!chatCollapsed && (
                <div className="h-[60vh] md:h-[52vh] overflow-y-auto pr-1" ref={listRef}>
                ...
                >
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      placeholder="Type your expense… (e.g., Pizza 900)"
              ...
              {recent.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm border rounded-md">
                    ...
                    <tbody>
                      {recent.map((it) => (
                        <tr key={it.expenseId} className="border-t">
                          <td className="px-3 py-2">{it.date}</td>
                          <td className="px-3 py-2">{it.category}</td>
                          <td className="px-3 py-2 text-right">₹{Number(it.amount).toLocaleString()}</td>
                          <td className="px-3 py-2 truncate max-w-[24ch]" title={it.rawText}>{it.rawText}</td>
        ...
        <div className="md:col-span-4 space-y-4">
          ...
          <Card>
            <CardContent>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-medium text-muted-foreground">Category breakdown</h2>
              </div>
